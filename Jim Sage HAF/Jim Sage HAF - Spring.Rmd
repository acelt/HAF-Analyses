...
output_filename <- "JimSage_spring"
...
strata <- "JimSage_strata_spring"

# name of sampled terradat points file in analysis gdb
terradat_file <- "td_UpperRaftRiverValley_Spring_final"

# name of sampled lmf points file in analysis gdb
#lmf_file <- "lmf_UpperRaftRiverValley_Spring_S3SeasonalHabitat"

# name of design points file in analysis gdb
design_file <- "design_UpperRaftRiverValley_Spring_final"
...
# theres two erroneous plot here: Site-01 and Grease-100 outside reporting unit without a wgtcat, need to remove
...
head(sampled_points[sampled_points$unique_id %notin% design_points$unique_id,])
...
# theres no rated LMF plots for this reporting unit
```
...
aim.analysis::weight_aimlmf(aim_points = aim_points,
                            ...
                            lmf_idvar <- "unique_id",
                            aim_fatevar <- "fate",
                            wgtcat_var <- as.character(wgtcat_var),
                            segments <- NULL,
                            segment_var <- "segment",
                            verbose <- FALSE)
...
###### Analysis ######
```{r}
all_points <- aim_points[, c("unique_id", "x", "y")]
...
fates <- unique(aim_points[["fate"]])

point_counts <- do.call(rbind,
                        lapply(X = split(aim_points@data[, c("wgtcat", "fate")], f = aim_points@data$wgtcat),
                               fates = fates,
...
all_points <- aim_points[, c("unique_id", "fate", "wgtcat", "source")]
...
wgtcat_summary$area <- as.numeric(wgtcat_summary$area)
wgtcat_summary$observed_area <- as.numeric(wgtcat_summary$observed_area)
...
```
# Merge wgtcatsummary with polygons
```{r}
