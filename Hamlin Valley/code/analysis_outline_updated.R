...
library(spdplyr)
#### Defaults
name <- "Hamlin Valley Project Area"
ru <- "HamlinValleyProjectArea"
...
### Filenames
reporting_unit_filenames <- "Hamlin Valley.gdb/HamlinValleyProject.shp"

design_filenames <- c("UT_CedarCityFO_2017_2021SDD.gdb")

terradat_filename <- "Hamlin Valley.gdb/hamlinvalley_terradat_points_strtm_segfinal"

additional_points_filename <- "Hamlin Valley.gdb/hamlinvalley_lmf_points_seg_strtm"

benchmarks_filename <-  "Hamlin Valley Benchmarks Project Area_V3.xlsm"
...
output_filename <- "HamlinValley_benchmarks2019"
...
names(reporting_units_polygons@data)[names(reporting_units_polygons@data) == "HV_name"] <- "Reporting.Unit"
...
# This isn't technically necessary, but it grabs only points that end in the year, "SEZs" or "Habitat"
# commenting this out because not sage grouse analysis right now
# terradat_points <- terradat_points[grepl(terradat_points@data[["ProjectName"]], pattern = "San Luis.*(SEZs|Habitat|\\d)$"), ]
...
data_tall <- tidyr::pivot_longer(data = tdat_df[, c("unique_id", aim_lmf , indicator_vars_present)],
                                 cols = tidyselect::one_of(indicator_vars_present), 
                                 names_to = "indicator.var",
                                 values_to = "value")

# merge in eval tab from benchmarks excel
...
data_tall <- merge(x = data_tall,
                   y = benchmarks,
                   by = c("indicator.var","Benchmark.Group"),
                   all.x = FALSE)
...
hamlinvalley_design_points <- sf::st_read(dsn = paste(path_spatial, "Hamlin Valley.gdb", sep = "/") ,
                           layer = "Terra_Sample_Points_Clip_Seg_Strtm",
                           stringsAsFactors = FALSE)
...
if (!all(unique(terradat_benchmarked[["PrimaryKey"]]) %in% terradat_points$unique_id)) {
  warning("NOT ALL RATED AIM PLOTS ARE PRESENT IN sampled_points!")
}
# 2 NAs in the terradat_benchmarked
terradat_benchmarked <- dplyr::filter(terradat_benchmarked,
                               !is.na(unique_id))

all(terradat_points$unique_id %in% terradat_benchmarked[["PrimaryKey"]]) 
length(terradat_points$unique_id)
length(unique(terradat_benchmarked$PrimaryKey))

# Where is this extra point coming from?

extra_points <- terradat_benchmarked[!(terradat_benchmarked[["PrimaryKey"]] %in% terradat_points@data[["unique_id"]]), ]

# Looks like there is an NA primary key for some reason in terradat_benchmarked. Will get rid of it

terradat_benchmarked <- terradat_benchmarked %>% filter(!is.na(PrimaryKey))
...
names(hamlinvalley_design_points)
names(terradat_points)
...
split_estimates <- split(analysis,  analysis[, c("Indicator")])
                    ...
                    categories = c("Not Meeting", "Uknown/Irrelevant", "Total", "Meeting"),
                                                              FUN = function(X, categories){
                                                                estimates <- X
                                                                # Junk!!!!! Keep only the good variables
                                                                string_variables <- c("Type", "Subpopulation", "Indicator", "Category")
...
terradat_benchmarked_wide <-  terradat_benchmarked[, c("unique_id", "Indicator", "Condition.Category")] %>%
  tidyr::pivot_wider(id_cols = "unique_id", names_from = "Indicator", values_from = "Condition.Category")
...
names(all_points@data) <- c("PrimaryKey", "Final Designation", "Weight Category", "Source", "LMF segment", "aim", "lmf", "observed", 
                            "relwgt", "Weight", "Reporting Unit","Non-Noxious Perennial Forb Cover","Non-Noxious Perennial Grass Cover","Noxious Annual Grass Cover","Sagebrush Cover","Percent in Gaps 101-200 cm","Percent in Gaps more than 200 cm","Protected Surface Soil Stability Rating","Unprotected Surface Soil Stability Rating")

all_points <- all_points[, c("PrimaryKey", "Final Designation", "Weight Category", "Source", "LMF segment", 
                              "Weight", "Reporting Unit","Non-Noxious Perennial Forb Cover","Non-Noxious Perennial Grass Cover","Noxious Annual Grass Cover","Sagebrush Cover","Percent in Gaps 101-200 cm","Percent in Gaps more than 200 cm","Protected Surface Soil Stability Rating","Unprotected Surface Soil Stability Rating")]
...
analysis1 <-subset(analysis, Rating=="Meeting"|Rating =="Not Meeting")

# add in zeros for not meeting
analysis_zero <-  analysis1
analysis_zero$Rating <- "Not Meeting"
...
analysis1 <-  rbind(analysis1, analysis_zero[13,])
...
ggplot2::ggplot(data = analysis1,
  ...
  labs(x = "",
       title = "Hamlin Valley Project Area")+
  scale_fill_manual(values = c("#FF0000",
                               "#3CE922"))+
  scale_y_continuous(labels = comma)

ggsave(device = "jpeg", dpi = 400, width = 17, filename = "hamlin_projectarea.jpeg", path = output_path)
