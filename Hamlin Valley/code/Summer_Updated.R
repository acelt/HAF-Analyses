...
library(spdplyr)
#### Defaults
name <- "Hamlin Valley Project Area Summer SUA"
ru <- "Summer"
...
### Filenames
reporting_unit_filenames <- "Hamlin Valley.gdb/summer.shp"

design_filenames <- c("UT_CedarCityFO_2017_2021SDD.gdb")

terradat_filename <- "Hamlin Valley.gdb/hamlinvalley_terradat_points_strtm_seg_summer"

additional_points_filename <- "Hamlin Valley.gdb/hamlinvalley_lmf_points_seg_strtm_summer"

benchmarks_filename <-  "Hamlin Valley Benchmarks Summer SUA.xlsm"
...
output_filename <- "HamlinValley_summer_benchmarks2019"
...
# terradat_points@data[["Benchmark.Group"]] <- terradat_points@data[["ProjectArea"]] this line isnt doing anything
...
wgtcat_df <-sf::st_read(dsn = paste(path_spatial, "Hamlin Valley.gdb", sep = "/"),
                        layer = "AIM_LMF_strata_union_summer",
                        stringsAsFactors = FALSE)
...
hamlinvalley_design_points <-  hamlinvalley_design_points[, c("unique_id", "fate", "wgtcat", "segment")]

# no rejections so dont need design points
...
## Commenting out everything relevant to LMF
...
analysis1 <-  rbind(analysis1, analysis_zero[c(1:3),])
...
ggplot2::ggplot(data = analysis1,
  ...
  labs(x = "",
       title = "GRSG Summer Seasonal Use Area")+
  scale_fill_manual(values = c("#FF0000",
                               "#3CE922"))+
  scale_y_continuous(labels = comma)

ggsave(device = "jpeg", dpi = 400, width = 17, filename = "hamlin_summer.jpeg", path = output_path)
