---
title: "Greater Curlew HAF Script using AIM and LMF data - Spring SUA"
author: "Alex Traynor"
date: "10/6/2021"
...
```
######## The next steps were also done in Arc ##########
...
output_path <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Greater Curlew HAF\\outputs\\"

# GDB within the output_path for writing points and wgtcat spatial outputs
output_gdb <- "Analysis Outputs.gdb"

# Filename for ratings from analysis requestor, normally a .xlsx
ratings_filename <- "Copy of HAF_ID_UT_2021_GreaterCurlewID_SiteScale_RATINGS_05-11-2021.xlsx"

# Core filename for all outputs for this reporting unit
output_filename <- "GreaterCurlew_Spring"
...
observed_fates = c("TS", "Target Sampled", "Sampled", "TargetSampled")

# The fates in sample designs which correspond to unneeded
# (e.g. unused oversample or points designated for future sampling)
invalid_fates = c("NN", NA, "Non Needed", "", "Not Sampled", "NT", "NotEval", "NonTarget")
...
path_spatial <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Greater Curlew HAF\\"

# The geodatabase containing all the spatial files
analysis_gdb <- "Analysis Inputs.gdb"

### The filepath to the folder where we have the Excel stuff
path_tabular <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Greater Curlew HAF\\"

### Spatial file names
# name of post-stratification file for this reporting unit
strata <- "strata_UT_ID_SUA_merge_Spring_GreaterCurlew_aea"

# name of sampled terradat points file in analysis gdb
terradat_file <- "tdat_UT_ID_SUA_merge_Spring_GreaterCurlew_aea"

# name of sampled lmf points file in analysis gdb
lmf_file <- "lmf_UT_ID_SUA_merge_Spring_GreaterCurlew_aea"

# name of design points file in analysis gdb
design_file <- "design_UT_ID_SUA_merge_Spring_GreaterCurlew_aea" # this doesnt contain LMF UT rejections
...
# in this case the UT point does not have a related design record
...
############################################################
```{r, eval=FALSE}
## Add check here for when there's no lmf points 
if(nrow(lmf_points)>0){
  weight_info_aimlmf <- aim.analysis::weight_aimlmf(aim_points = aim_points,
                              lmf_points = lmf_points,
                              aim_idvar = "unique_id",
                              lmf_idvar = "unique_id",
                              aim_fatevar = "fate",
                              observed_fates = observed_fates,
                              invalid_fates = invalid_fates,
                              wgtcat_var = "wgtcat",
                              segments = NULL,
                              segment_var = "segment",
                              verbose = TRUE,
  ...
  weight_info <- aim.analysis::weight.gen(pts = aim_points,
                       pts.fatefield = "fate", #pts.fatefield
                       pts.groupfield = wgtcat_var, #"WEIGHT.ID"
                       frame.spdf = wgtcat_spdf,
                       frame.groupfield = wgtcat_var, #designstratumfield
                       target.values = observed_fates,
                       unknown.values = NULL,
                       nontarget.values = NULL,
                       inaccessible.values = c("IA", "Inaccessible"),
                       unneeded.values = invalid_fates)
  
  # reorganise weight info to match outputs from weights_aimlmf?
  point_weights <- weight_info[["point.weights"]]
  point_weights$wgt <- point_weights$WGT
...
aim_points$segment <- NA
aim_points$area_ratio <-  NA
...
all_points[["reporting_unit"]] <- output_filename
point_weights[["reporting_unit"]] <- output_filename
...
goodman_cis <- aim.analysis::goodman_cis(counts = adjusted_counts)
...
all_points <- sp::merge(x = aim_points,
                        y = point_weights,
                        all = FALSE)
...
names(all_points@data) <- c("plot_id", "fate", "wgtcat", "segment", "area_ratio", "reporting_unit","x","y",  "source",   "aim", "lmf", "observed","relwgt",  
                            "wgt", "rating")
