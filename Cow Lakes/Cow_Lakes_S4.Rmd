...
seasonal_habitat_type = "UplandSummer.S4"
...
ratings <- ratings[!(ratings$UplandSummer.S4==""), ]
...
# Ive already combined te 3 designs in Arc
...
## There are more plots in the sample design than in terradat for this season. 
# I am assuming they were dropped because temporally did not meet summer criteria, checking below - nope, its because there are dups 
...
if (!all(ratings[["PrimaryKey"]] %in% sampled_points$PrimaryKey)) {
...
ratings[["PrimaryKey"]][ratings[["PrimaryKey"]] %notin% sampled_points$PrimaryKey] # 1 LMF plot - 20171673103220B2 was rateed but not in sampled points - this is the LMF point dropped since it landed on private
...
ratings_tdat <- ratings %>% select(PrimaryKey, Plot.ID, Source, Project, UplandSummer.S4, Visit.Date)  # trim to columns needed
