---
title: "Upper Raft River HAF - Summer"
author: "Alex Traynor"
date: "1/19/2021"
...
output_path <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Upper Raft River HAF\\output\\UpperRaftRiver_summer\\"

# Filename for ratings from analysis requestor
ratings_filename <- "HAF_ID_2020_UpperRaftRiverValley_SiteScale_RESULTS_01-19-2021.xlsx"

# Core filename for all outputs
output_filename <- "UpperRaftRiver_summer"

# Reporting Unit name - corresponds to column name in ratings spreadsheet
sua_name <- "Suitability_Upland_Summer_S4"
...
path_spatial <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Upper Raft River HAF\\inputs\\spatial\\Jim Sage HAF\\"

# The geidatabase containing all the spatial files
analysis_gdb <- "Jim Sage HAF.gdb"

### The filepath to the folder where we have the Excel stuff
path_tabular <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Upper Raft River HAF\\inputs\\"

### Spatial file names
# name of postratification file for this reporting unit
strata <- "strata_SummerSUA_finescale"

# name of sampled terradat points file in analysis gdb
terradat_file <- "td_SummerSUA_finescale"

# name of sampled lmf points file in analysis gdb
lmf_file <- "LMF_SummerSUA_finescale"

# name of design points file in analysis gdb
design_file <- "design_SummerSUA_finescale"
...
jim_sage_design <- "JimSage_designpoints_summer"
```
...
wgtcats["wgtcat_name"] <- wgtcats[[wgtcat_name]]
wgtcats[["area_hectares"]] <- wgtcats[[wgtcat_area_var]]

wgtcats <- wgtcats[,c("wgtcat","wgtcat_name","area_hectares")]
```

#### Plot Ratings
```{r}
ratings <- readxl::read_excel(path = paste0(path_tabular, ratings_filename),
                             sheet = ratings_sheet_name) ## getting an error here based on column types so adding this in explicitly

# removing any NAs and unrated plots:
ratings <- ratings[!is.na(ratings[[sua_name]]),]
```
...
# Same for the JIm Sage
### add design points
js_design_points <- sf::st_read(dsn = paste(path_spatial, analysis_gdb, sep = "/"),
                           layer = jim_sage_design,
                           stringsAsFactors = FALSE)
## Standardize projection
js_design_points <- sf::st_transform(js_design_points,
                                crs = projection)

js_design_points <- sf::st_zm(js_design_points,
                            drop = TRUE,
                            what = "ZM") # simplify becuase theres Z/M values

## Make spatial 
js_design_points <- methods::as(js_design_points, "Spatial")
```
...
js_design_points@data[["unique_id"]] <- js_design_points@data[[aim_idvar]]
js_design_points@data[["fate"]] <- js_design_points@data[[aim_fatevar]]
js_design_points@data[["wgtcat"]] <- js_design_points@data[[wgtcats_var]]
js_design_points@data[["segment"]] <- js_design_points@data[[segment_var]]
js_design_points@data[["area_ratio"]] <- js_design_points@data[[lmf_area_var]]
js_design_points@data <- js_design_points@data[,c("unique_id", "fate","wgtcat","segment", "area_ratio")]

design_points <- rbind(design_points, js_design_points)
```
...
sampled_points[sampled_points$unique_id %notin% design_points$unique_id,] # only LMF

design_points$unique_id[design_points$fate %in% observed_fates] %notin% sampled_points$unique_id
...
# There one extra point here form the salt lake desing also named site 01 - in wgtcat 17 - this wasnt rated so lets remove
sampled_points <- sampled_points[!(sampled_points$unique_id == "Site-01" & sampled_points$wgtcat == 17),]
```
# Now combine:
...
# winter has an AIM point: "201" so nedd to amend this code
lmf_points <- aim_points
lmf_points <- subset(lmf_points, grepl("^[0-9]{14}|^[0-9]{13}",lmf_points@data$unique_id))

aim_points <- subset(aim_points, !grepl("^[0-9]{14}|^[0-9]{13}",aim_points@data$unique_id))

# theres 2 LMF points that are not on BLM land here: 20141631104808B1, 20141631104808B2 need  to remove these
lmf_points <- lmf_points[lmf_points$unique_id != "20141631104808B1" & lmf_points$unique_id != "20141631104808B2",]
```
