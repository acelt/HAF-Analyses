---
title: "Dry Valley Jack Mountain"
output:
  html_document:
    df_print: paged
---
...
ratings <- ratings %>%
  ...
  dplyr::select(c(Project,`Plot ID`, `Visit Date`,sua_name))
...
design1_points <- sf::st_read(dsn = paste(path_spatial, "Analysis.gdb", sep = "/"),
                           ...
                           stringsAsFactors = FALSE)
## Standardize projection
design1_points <- sf::st_transform(burly_design_points,
                                crs = projection)
## Make spatial 
design1_points <- methods::as(burly_design_points, "Spatial")
...
aim_points[["source"]] <- "Modified HAF Plot"
...
#analysis <- factor(analysis,levels(analysis)[c(2,1,3)])
