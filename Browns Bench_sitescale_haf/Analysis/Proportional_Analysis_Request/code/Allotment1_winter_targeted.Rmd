---
title: "Brown Bench Site Scale HAF Weighted Analysis - Allotment1 - Winter"
output:
  html_document:
    df_print: paged
---
...
output_path <- "C:\\Users\\alaurencetraynor\\Documents\\2019\\Analysis\\BrownsBenchSiteScaleHAF\\bb_sitescale_haf\\Analysis\\Proportional_Analysis_Request\\outputs\\Allotment1\\Winter"
...
output_filename <- "brownsbench_analysis_allot1_winter_targeted"
...
wgtcat_df <-sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                        layer = "poststrata_winter_allot1",
...
sampled_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "All_sampled_plots_winter_allot1",
                           stringsAsFactors = FALSE)
...
burly_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "burly_design_points_winter_allot1",
                           stringsAsFactors = FALSE)
## Standardize projection
...
mhaf2018_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "mhaf2018_design_points_winter_allot1",
                           stringsAsFactors = FALSE)
## Standardize projection
...
design_points <- rbind(burly_design_points[, c("unique_id", "fate", "wgtcat", "segment")],mhaf2018_design_points[, c("unique_id", "fate", "wgtcat", "segment")]) 
...
all_points[["reporting_unit"]] <- "Allotment1_Winter"
point_weights[["reporting_unit"]] <- "Allotment1_Winter"
...
rating_summaries <- lapply(X = split(observed_points, observed_points[["Suitability.S6"]]),
                           FUN = function(X){
                             data.frame(habitat_type = X[["reporting_unit"]][1],
                                        indicator = X[["indicator"]][1],
                                        rating = X[["Suitability.S6"]][1],
                                        count = nrow(X),
