---
title: "Brown Bench Site Scale HAF Weighted Analysis - Winter"
output:
  html_document:
    df_print: paged
---
...
output_path <- "C:\\Users\\alaurencetraynor\\Documents\\2019\\Analysis\\BrownsBenchSiteScaleHAF\\bb_sitescale_haf\\Analysis\\Proportional_Analysis_Request\\outputs\\Winter"
...
output_filename <- "brownsbench_analysis_winter"
...
# (e.g. unused oversample or oints designated for future sampling)
invalid_fates = c("NN", NA, "Non Needed", "", "Not Sampled")

# The LMF segments polygons
# DONT NEED HERE SINCE WE JOINED IN aRC
segments <- NULL
...
ratings <- ratings %>%
  filter(!is.na(Suitability.S6),
                Suitability.S6 != "")%>%  # Only interested in the spring plots
  select(-c(Suitability.S2,Suitability.S4,Suitability.S5,Suitability.S3))
```
...
sampled_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "All_sampled_plots_winter",
                           stringsAsFactors = FALSE)
...
burly_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "burly_design_points_winter",
                           stringsAsFactors = FALSE)
## Standardize projection
...
elko_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "elko_design_points_winter",
                           stringsAsFactors = FALSE)
## Standardize projection
...
jarbidge_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "jarbidge_design_points_winter",
                           stringsAsFactors = FALSE)
## Standardize projection
...
mhaf2017_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "mhaf2017_design_points_winter",
                           stringsAsFactors = FALSE)
## Standardize projection
...
mhaf2018_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "mhaf2018_design_points_winter",
                           stringsAsFactors = FALSE)
## Standardize projection
...
missing <- ratings[!ratings[["Plot_ID"]] %in% sampled_points$unique_id,]
missing
# ELKO-MtnSage-226 is rated but not within winter SUA
# lets remove from ratings for now

ratings <-  ratings[ratings$Plot_ID != "ELKO-MtnSage-226",]
sampled_points <- sampled_points[sampled_points$unique_id %in% ratings[["Plot_ID"]], ] 
```
# Now combine:
...
all_points$Suitability.S6 <- as.character(all_points$Suitability.S6) # analyze function giving "lossy cast error" - need to convert factors to characters

# Add in the season (this is defined up above) e.g. "S-4 Upland Summer Late Brood-rearing Habitat"
all_points[["reporting_unit"]] <- "Winter"
point_weights[["reporting_unit"]] <- "Winter"
...
all_points <- sp::merge(x = all_points,
                        y = ratings[, c("Plot_ID", "Suitability.S6")],
                        by.x = "unique_id",
                        by.y = "Plot_ID",
                        all.x = TRUE)
