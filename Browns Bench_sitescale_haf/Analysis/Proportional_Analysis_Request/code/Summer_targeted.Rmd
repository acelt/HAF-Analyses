...
output_filename <- "brownsbench_analysis_summer_targeted"
...
# And analyze
# lets use the raw code here to exclude cat.analysis so we can incorporate the targeted points
# Just need the ratings and the weights in one place
# This also eliminates the weights for unrated (NT and IA) plots
# because they don't match anything in all_points (the name should be all_observed_points)
observed_points <- merge(x = all_points,
                         y = point_weights[, c("unique_id", "wgt")],
                         by = "unique_id")

# Sure, this could be a dplyr::summarize() but I'm doing it in base
rating_summaries <- lapply(X = split(observed_points, observed_points[["Suitability.S4"]]),
                           FUN = function(X){
                             data.frame(habitat_type = X[["reporting_unit"]][1],
                                        indicator = X[["indicator"]][1],
                                        rating = X[["Suitability.S4"]][1],
                                        count = nrow(X),
                                        estimated_observed_hectares = sum(X[["wgt"]]),
                                        stringsAsFactors = FALSE)
                           })
# Turn that list into a single data frame
analysis <- do.call(rbind,
                    rating_summaries)

# This is already only observed point weights, so this is observed area
total_observed_area <- sum(analysis[["estimated_observed_hectares"]])

# Nothing groundbreaking, just converting to proportions
analysis[["estimated_proportion_of_area"]] <- analysis[["estimated_observed_hectares"]] / total_observed_area

# And using those to adjust the counts to feed into the math for Goodman's confidence intervals
analysis[["adjusted_count"]] <- sum(analysis[["count"]]) * analysis[["estimated_proportion_of_area"]]

# Breaking this out as an object so I don't need to change the following code
adjusted_counts <- analysis[["adjusted_count"]]
...
goodman_cis[["rating"]] <- analysis[["rating"]]

# Add it to the summary!
analysis <- merge(x = analysis,
              y = goodman_cis[, c("rating", "lower_bound", "upper_bound")])
```
...
analysis[["estimated_proportion_of_area"]] <- analysis[["estimated_proportion_of_area"]] * 100

# Calculate the hectares using the Goodman binomial confidence intervals from percentage
analysis[[paste0("LCB", confidence, "Pct.U.Goodman")]] <- analysis[["lower_bound"]] / 100 * analysis[["estimated_observed_hectares"]]
analysis[[paste0("UCB", confidence, "Pct.U.Goodman")]] <- analysis[["upper_bound"]] / 100 * analysis[["estimated_observed_hectares"]]

# dont need adjusted counts
analysis <-  analysis[,-7]
