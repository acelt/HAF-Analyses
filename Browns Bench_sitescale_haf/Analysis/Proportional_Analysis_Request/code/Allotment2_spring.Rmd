---
title: "Brown Bench Site Scale HAF Weighted Analysis - Allotment 2 - Spring"
output:
  html_document:
    df_print: paged
---
...
output_path <- "C:\\Users\\alaurencetraynor\\Documents\\2019\\Analysis\\BrownsBenchSiteScaleHAF\\bb_sitescale_haf\\Analysis\\Proportional_Analysis_Request\\outputs\\Allotment2\\Spring"
...
output_filename <- "brownsbench_analysis_allot2_spring"
...
wgtcat_df <-sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                        layer = "poststrata_spring_allot2",
...
colnames(wgtcat_spdf@data)[3] <- "wgtcat"
...
sampled_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "All_sampled_plots_spring_allot2",
                           stringsAsFactors = FALSE)
## Standardize projection
...
burly_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "burly_design_points_spring_allot2",
                           stringsAsFactors = FALSE)
## Standardize projection
...
### No Elko points in here
       ...
       #                    layer = "elko_design_points_spring_allot2",
            #              stringsAsFactors = FALSE)
## Standardize projection
...
#jarbidge_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
       #                    layer = "jarbidge_design_points_spring_allot2",
       #                    stringsAsFactors = FALSE)
## Standardize projection
...
mhaf2017_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "mhaf2017_design_points_spring_allot2",
                           stringsAsFactors = FALSE)
## Standardize projection
...
mhaf2018_design_points <- sf::st_read(dsn = paste(path_spatial, "BrownsBench Analysis.gdb", sep = "/"),
                           layer = "mhaf2018_design_points_spring_allot2",
                           stringsAsFactors = FALSE)
## Standardize projection
...
lmf_points <- subset(lmf_points, grepl("^20",lmf_points@data$unique_id)) # actually no lmf points in here
# SO ADD FAKE IDS
lmf_points["unique_id"] <- nrow(lmf_points)
...
aim.analysis::weight_gen(aim_points = aim_points,
...
```{r}
# no lmf points so we can add dummy variables to LMF 
...
all_points[["reporting_unit"]] <- "Allotment2_Spring"
point_weights[["reporting_unit"]] <- "Allotment2_Spring"
