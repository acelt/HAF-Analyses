---
title: "Dry Valley Jack Mountain Spring"
output:
  html_document:
    df_print: paged
---
...
output_path <- "C:\\Users\\alaurencetraynor\\Documents\\2020\\Analysis\\ORWA\\Analysis_Outputs\\"

# Filename for ratings from analysis requestor
ratings_filename <- "Spring_ARTR.xls"
ratings_filename_lmf <- "Spring_ARTR_LMF.xls"

plot_list_filename <- "Copy of HAF_Oregon_2020_DVJM_SiteScale_NOCANALYSISREQUEST_20200716.xlsx"

# Core filename for all outputs
output_filename <- "DryValleyJackMtn_spring_landcart1"
...
path_spatial <- "C:\\Users\\alaurencetraynor\\Documents\\2020\\Analysis\\ORWA\\"
...
ratings <- readxl::read_excel(path = paste0(path_tabular, ratings_filename),
                             sheet = seasonal_habitat_type) ## getting an error here based on column types so adding this in explicitly

lmf_ratings <- readxl::read_excel(path = paste0(path_tabular, ratings_filename_lmf))

ratings[["unique_id"]] <- ratings[[ratings_idvar]]
ratings[["rating"]] <- ratings[[sua_name]]
  
lmf_ratings[["unique_id"]] <- lmf_ratings[[lmf_idvar]]
lmf_ratings[["rating"]] <- lmf_ratings[[sua_name]]

ratings <- ratings[,c("unique_id","rating")]
lmf_ratings <- lmf_ratings[,c("unique_id","rating")] 
# removing any NAs and unrated plots:
ratings <- rbind(ratings,lmf_ratings)
ratings[["indicator"]] <- "AH_Sagebrush_Cover"

plot_list <- readxl::read_excel(path = paste0(path_tabular, plot_list_filename),sheet = "S-3 Nesting Early Brood Rearing")

ratings <- ratings[ratings$unique_id %in% plot_list$Plot_Identifier,]
```
...
sampled_points <- sampled_points[sampled_points$unique_id %in% ratings[["unique_id"]], ] 
```
# Now combine:
...
results graphic
```{r}
spring_analysis <- read.csv("C:\\Users\\alaurencetraynor\\Documents\\2020\\Analysis\\ORWA\\ORWA Dry Valley Jack Mountain HAF\\output\\DryValleyJackMtn_spring_analysis_2020-08-26.csv", check.names = FALSE)
summer_analysis <- read.csv("C:\\Users\\alaurencetraynor\\Documents\\2020\\Analysis\\ORWA\\ORWA Dry Valley Jack Mountain HAF\\output\\DryValleyJackMtn_summer_analysis_2020-08-26.csv", check.names = FALSE)
winter_analysis <- read.csv("C:\\Users\\alaurencetraynor\\Documents\\2020\\Analysis\\ORWA\\ORWA Dry Valley Jack Mountain HAF\\output\\DryValleyJackMtn_winter_analysis_2020-08-26.csv", check.names = FALSE)

all_analysis <- rbind(spring_analysis,summer_analysis,winter_analysis)

all_analysis$Rating <- as.factor(all_analysis$Rating)
all_analysis$Rating <-  factor(all_analysis$Rating, levels = c("Unsuitable", "Marginal","Suitable"))
all_analysis <- all_analysis[order(all_analysis$Rating),]

library(ggplot2)

my.palette <- c("red","yellow","green")
#plot_data <- all_analysis
plot_data <- analysis

analysis$Rating <- as.factor(analysis$Rating)
analysis$Rating <-  factor(analysis$Rating, levels = c("Unsuitable", "Marginal","Suitable"))
analysis <- analysis[order(analysis$Rating),]

p <- ggplot2::ggplot(data = plot_data, aes(x = `Estimated hectares`, y=Rating, fill = Rating)) +
...
p
ggsave("DVJM_analysis_springARTR.png", device = "png", dpi = 500)
```
