...
#### Overview
Three category HAF analysis for the Benchgrass allotment in the Piendale FO, WY.
...
output_path <- "C:/Users/alaurencetraynor/Documents/2019/Analysis/Pinedale/outputs/May 2020"

# Filename for ratings from analysis requestor
ratings_filename <- "HAF_Wyoming_2020_Benchgrass_SiteScale_NOCANALYSISREQUEST_Clean.xlsx"

# Core filename for all outputs
output_filename <- "PinedaleHAF_Summer"
...
# The current workflow is to join LMF segments to points in ArcMap so segment polys are only needed to calculate adjusted weights for segmens which have been cut in half
segments <- NULL
...
wgtcats_var <- "StrataID"
```

### The filepath to the folder where we have all the spatial data
```{r}
path_spatial <- "C:/Users/alaurencetraynor/Documents/2019/Analysis/Pinedale/inputs/spatial"
```
### The filepath to the folder where we have the Excel stuff
```{r}
path_tabular <- "C:/Users/alaurencetraynor/Documents/2019/Analysis/Pinedale/inputs/tabular"
```
...
#### Strata polygons
```{r}
wgtcat_df <-sf::st_read(dsn = paste(path_spatial, "Benchgrass_QA_Strata_Creation.gdb", sep = "/"),
                        layer = "Pinedale_Strata_Benchgrass_Final_latebrood",
...
ratings <- ratings %>%
  dplyr::filter(!is.na(S4),
                sua_name != "")%>%  # Only interested in the sua plots
  dplyr::select(c("AIM_or_LMF","Plot_Identifier", "Sample_Date",sua_name))
...
```{r}
sampled_points <- sf::st_read(dsn = paste(path_spatial, "Benchgrass_QA_Strata_Creation.gdb", sep = "/"),
                           layer = "terradat_latebrood",
                           stringsAsFactors = FALSE)
...
#### Design points and LMF
```{r}
### design
design_points <- sf::st_read(dsn = paste(path_spatial, "Benchgrass_QA_Strata_Creation.gdb", sep = "/"),
                           layer = "design_points_latebrood_final",
                           stringsAsFactors = FALSE)
## Standardize projection
...
# LMF points:
lmf_points <- sf::st_read(dsn = paste(path_spatial, "Benchgrass_QA_Strata_Creation.gdb", sep = "/"),
                           layer = "lmf_points_latebrood_final",
                           stringsAsFactors = FALSE)
## Standardize projection
...
length(design_points$unique_id[design_points$fate %in% observed_fates]) # 21 observed plots in design - 2019 observed plots are included here and not in sampled points
...
if (!all(ratings[["Plot_Identifier"]][ratings[["AIM_or_LMF"]] == "AIM"] %in% sampled_points$unique_id)) {
  warning("NOT ALL RATED AIM PLOTS ARE PRESENT IN sampled_points!")
  
} #warning here (probably from missing 2019 plots, lets check
ratings[["Plot_Identifier"]][ratings[["AIM_or_LMF"]] == "AIM"] %in% sampled_points$unique_id

# actually looks like a case of trading "-" for "_" lets replace to match how the deisgn files treats it (with a "-")
ratings[["Plot_Identifier"]] <- gsub("_(?=\\d\\d\\d)","-",ratings[["Plot_Identifier"]], perl = TRUE)

length(design_points$unique_id[design_points$fate %in% observed_fates] %in% ratings[["Plot_Identifier"]][ratings[["AIM_or_LMF"]] == "AIM"])

all(sampled_points$unique_id %in% ratings[["Plot_ID"]]) 

# filter sampled plots by those that were rated:
sampled_points <- design_points[design_points$unique_id %in% ratings[["Plot_Identifier"]], ]
sampled_points@data[["fate"]] <- observed_fates[1]

lmf_points <- lmf_points[lmf_points$unique_id %in% ratings[["Plot_Identifier"]], ]
```
...
all_points <- merge(x = all_points@data,
                    y = ratings,
                    by.x = "unique_id",
                    by.y = "Plot_Identifier",
                    all.x = FALSE)

# Add in the indicator, which for these will always be "Habitat suitability"
all_points[["indicator"]] <- "Habitat suitability"
all_points$S4 <- as.character(all_points$S4) # analyze function giving "lossy cast error" - need to convert factors to characters
...
all_points <- sp::merge(x = all_points,
                        y = ratings[, c("Plot_Identifier", "S4")],
                        by.x = "unique_id",
                        by.y = "Plot_Identifier",
                        all.x = TRUE)

names(all_points@data) <- c("plot_id", "fate", "wgtcat", "source", "segment", "aim", "lmf","observed",  "relwgt", 
...
analysis$Rating <- factor(analysis$Rating,levels(analysis$Rating)[c(2,1,3)])
...
```{r}
wgtcat_spdf <-sf::st_read(dsn = paste(path_spatial, "Benchgrass_QA_Strata_Creation.gdb", sep = "/"),
                        layer = "Pinedale_Strata_Benchgrass_Final_latebrood",
                        stringsAsFactors = FALSE)
## Standardize projection
