...
output_filename <- "PinedaleHAF_Winter"
...
wgtcat_df <-sf::st_read(dsn = paste(path_spatial, "Benchgrass_QA_Strata_Creation.gdb", sep = "/"),
                        layer = "Pinedale_Strata_Benchgrass_Final_winter",
...
design_points <- sf::st_read(dsn = paste(path_spatial, "Benchgrass_QA_Strata_Creation.gdb", sep = "/"),
                           layer = "design_points_winter_final",
                           stringsAsFactors = FALSE)
## Standardize projection
...
all_points$S6 <- as.character(all_points$S6) # analyze function giving "lossy cast error" - need to convert factors to characters
...
all_points <- sp::merge(x = all_points,
                        y = ratings[, c("Plot_Identifier", "S6")],
                        by.x = "unique_id",
                        by.y = "Plot_Identifier",
                        all.x = TRUE)
