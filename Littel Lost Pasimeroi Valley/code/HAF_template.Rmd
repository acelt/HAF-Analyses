---
title: "Name of Analysis and Reporting Unit"
output:
  html_document:
    df_print: paged
---
...
output_path <- "\\AnalysisName\\outputs\\ReportingUnit\\"

# Filename for ratings from analysis requestor
ratings_filename <- "HAF site scale ratings spreadsheet"

# Core filename for all outputs
output_filename <- "analysisname_reportingunit"

# Reporting Unit name - corresponds to column header in ratings spreadsheet
sua_name <- "Suitability: \r\nSpring-S3" or "Suitability: \r\nUpland Summer-S4" or "Suitability: \r\nWinter-S6"
...
```{r}
path_spatial <- "analysisname/inputs/spatial"
```
### The filepath to the folder where we have the Excel stuff
```{r}
path_tabular <- "analysisname/inputs/tabular"
```
...
#### Strata polygons
```{r}
wgtcat_df <-sf::st_read(dsn = paste(path_spatial, "Analysis.gdb", sep = "/"),
                        layer = "poststrata_reportingunit1",
                        stringsAsFactors = FALSE)
sf::st_geometry(wgtcat_df) <- NULL
wgtcats <- as.data.frame(wgtcat_df)[,c(1:3)] # should be careful here using colums number as reference
...
ratings1 <- subset(ratings[,c("Project","Plot ID", "Visit Date",sua_name)], !is.na(ratings[[sua_name]]) & ratings[[sua_name]] != "" & ratings[[sua_name]] != "0")
```

#### Sampled Points 
Make sure points and polygons do not include Z/M dimensions...
```{r}
sampled_points <- sf::st_read(dsn = paste(path_spatial, "Analysis.gdb", sep = "/"),
                           layer = "All_sampled_plots_reportingunit",
                           stringsAsFactors = FALSE)
...
#### Design points
```{r}
design1_points <- sf::st_read(dsn = paste(path_spatial, "Analysis.gdb", sep = "/"),
                           layer = "design1_design_points_reportingunit",
                           stringsAsFactors = FALSE)
## Standardize projection
...
# check to see if plot number/fates match up between design and sampled points
```
### QC
```{r}
...
all(sampled_points$unique_id %in% ratings[["Plot ID"]]) 

# filter sampled plots by those that were rated
# this should have been done in ArcMap already
...
# Depending on GIS prework - may need to separate out LMF points
...
adjusted_counts <- (sum(analysis[["NResp"]][analysis[["Category"]] != "Total"]) * analysis[["Estimate.P"]][analysis[["Category"]] != "Total"] / 100) # this formula is only suitable if there is only one objective/indicator per reporting unit - for analyses with multiple objectives per reporting unit use "split_estimates function"
...
wgtcat_summary <- wgtcat_summary[, c(wgtcats_var, "area", "area_units",
                                     ...
                                     "observed_proportion",
                                     "observed_area", "unobserved_area",
                                     "in_inference", "Stratum")]

# Add in where they're from
aim_points[["source"]] <- "Additional Points Design"
...
# Reformat for Idaho HAF spreadsheet
```{r}
date <- Sys.Date()
...
wgtcat_spdf <-sf::st_read(dsn = paste(path_spatial, "Analysis.gdb", sep = "/"),
                        layer = "poststrata_reportingunit",
                        stringsAsFactors = FALSE)
## Standardize projection
