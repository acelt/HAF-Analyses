---
title: "HAF Template Script using AIM and LMF data"
author: "Alex Traynor"
date: "6/23/2021"
...
# Geodatabase containing reporting units
reportingunit_gdb <-  "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Big Lost HAF GIS\\Big Lost HAF GIS.gdb"
#Name of each reporting unit feature class
spring_name <-  "Dissolve_OutFeatureClass_Spring_AntelopeFlats_BigLost_spring_S3SeasonalHabitat"
summer_name <- "Dissolve_OutFeatureClass_Summer_AntelopeFlats_BigLost_summer_S4SeasonalHabitat"
winter_name <-  "Dissolve_OutFeatureClass_Winter_AntelopeFlats_BigLost_winter_S6SeasonalHabitat"
finescale_name <- "AntelopeFlats_BigLost_FineScaleArea"

# Location of terradat
terradat_gdb <-  "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\TerrADat.gdb"
# location of lmf
lmf_gdb <-  "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\lmf_strata_segments_20191216.gdb"
# Name of LMF strata file
lmf_strata <- "strata_SMA_2020_clip"
# Location of ratings spreadsheet
ratings_xlsx <-  "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Big Lost HAF GIS\\1_Analysis\\HAF_ID_2021_AntelopeFlats_BigList_SiteScale_RESULTS_05-20-2021.xlsx"
# location of SDD SDE for analysis
sdd_path <-  "C:\\Users\\alaurencetraynor\\Documents\\2021\\DATA\\SDD SDE\\SDDSDE_Schema_2021_V2_ID.gdb"
# output location for SDD files
out_path <-  "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Big Lost HAF GIS\\2_Methods"
...
output_path <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Big Lost HAF GIS\\3_Results\\"

# GDB within the output_path for writing points and wgtcat spatial outputs
output_gdb <- "Antelope_BigLost_Results.gdb"

# Filename for ratings from analysis requestor, normally a .xlsx
ratings_filename <- "HAF_ID_2021_AntelopeFlats_BigList_SiteScale_RESULTS_05-20-2021.xlsx"

# Core filename for all outputs for this reporting unit
output_filename <- "Antelope_BigLost_Spring"

# Column name in ratings spreadsheet
ratings_col_name <- "Suitability Spring-S3"

# Sheet name in ratings Excel workbook)
ratings_sheet_name = "Plot Summary"
...
# in this case  the spreadhseet has some errors  in the PK field  (some are PLOTKEY instead..)\
# will check plotid to see if its unique and use that instead
ratings_idvar <- "Plot ID"

# The fates in sample designs which correspond to sampled/observed
observed_fates = c("TS", "Target Sampled", "Sampled", "Evaluated")

# The fates in sample designs which correspond to unneeded
# (e.g. unused oversample or points designated for future sampling)
invalid_fates = c("NN", NA, "Non Needed", "", "Not Sampled", "NT", "NotEval", "Non Target", "Not Eval", "NonTarget")

# The variable containing the LMF segment identity/membership
# This is found in the points and the segment polygons
#double  checked thes eare all unique
segment_var <- "segcode"
...
path_spatial <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Big Lost HAF GIS\\1_Analysis\\"

# The geodatabase containing all the spatial files
analysis_gdb <- "R Inputs.gdb"

### The filepath to the folder where we have the Excel stuff
path_tabular <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\ID\\Big Lost HAF GIS\\1_Analysis\\"

### Spatial file names
# name of post-stratification file for this reporting unit
strata <- "strata_AntelopeFlats_BigLost_spring_S3SeasonalHabitat_dissolve"

# name of sampled terradat points file in analysis gdb
terradat_file <- "tdat_AntelopeFlats_BigLost_spring_S3SeasonalHabitat_dissolve"

# name of sampled lmf points file in analysis gdb
lmf_file <-"lmf_AntelopeFlats_BigLost_spring_S3SeasonalHabitat_dissolve"

# name of design points file in analysis gdb
design_file <- "design_AntelopeFlats_BigLost_spring_S3SeasonalHabitat_dissolve"
...
# additional point
# in this case we have modified HAF points from Challis which are not in terradt or the SDD SDE
mod_haf_points_2018 <- "modhaf2018_design_AntelopeFlats_BigLost_spring_S3SeasonalHabitat_dissolve"
mod_haf_points_2019 <- "modhaf2019_design_AntelopeFlats_BigLost_spring_S3SeasonalHabitat_dissolve"
haf_fate <- "EvalStatus"
haf_id <- "PLOTID"
```
...
# Several of thewse point are deemed targetedand will not be used - the craters of the moon 2013 design and the 2018 state esd design, lets take them out now

ratings <- ratings[ratings$Project %notin% c("Idaho State ESD 2018", "Idaho CRMONM AIM 2013"),]

# nrow(ratings) == length(unique(ratings$`Plot ID`)) in this case plot  id is unique - lets use that 
```
...
```{r}
### add design points
haf_points_2018 <- sf::st_read(dsn = paste(path_spatial, analysis_gdb, sep = "/"),
                           layer = mod_haf_points_2018,
                           stringsAsFactors = FALSE)
## Standardize projection
haf_points_2018 <- sf::st_transform(haf_points_2018,
                                crs = projection)

haf_points_2018 <- sf::st_zm(haf_points_2018,
                            drop = TRUE,
                            what = "ZM") # simplify becuase theres Z/M values

## Make spatial 
haf_points_2018 <- methods::as(haf_points_2018, "Spatial")

### add design points
haf_points_2019 <- sf::st_read(dsn = paste(path_spatial, analysis_gdb, sep = "/"),
                           layer = mod_haf_points_2019,
                           stringsAsFactors = FALSE)
## Standardize projection
haf_points_2019 <- sf::st_transform(haf_points_2019,
                                crs = projection)

haf_points_2019 <- sf::st_zm(haf_points_2019,
                            drop = TRUE,
                            what = "ZM") # simplify becuase theres Z/M values

## Make spatial 
haf_points_2019 <- methods::as(haf_points_2019, "Spatial")
...
# Modified HAF
haf_points_2018@data[["unique_id"]] <- haf_points_2018@data[[haf_id]]
haf_points_2018@data[["fate"]] <- haf_points_2018@data[[haf_fate]]
haf_points_2018@data[["wgtcat"]] <- haf_points_2018@data[[wgtcats_var]]
haf_points_2018@data[["segment"]] <- haf_points_2018@data[[segment_var]]
haf_points_2018@data[["area_ratio"]] <- haf_points_2018@data[[lmf_area_var]]
haf_points_2018@data <- haf_points_2018@data[,c("unique_id", "fate","wgtcat","segment", "area_ratio")]

haf_points_2019@data[["unique_id"]] <- haf_points_2019@data[[haf_id]]
haf_points_2019@data[["fate"]] <- haf_points_2019@data[[haf_fate]]
haf_points_2019@data[["wgtcat"]] <- haf_points_2019@data[[wgtcats_var]]
haf_points_2019@data[["segment"]] <- haf_points_2019@data[[segment_var]]
haf_points_2019@data[["area_ratio"]] <- haf_points_2019@data[[lmf_area_var]]
haf_points_2019@data <- haf_points_2019@data[,c("unique_id", "fate","wgtcat","segment", "area_ratio")]

# NOW HAVE TO RBIND
###### theres 1 rejected plot in the haf points which was rejected due to being in the incrrect strata - this we will label as Non target (AND REMOVE)

haf_points <- rbind(haf_points_2018,haf_points_2019)

# combine lmf and aim points for ratings check

sampled_points <- rbind(lmf_points, sampled_points,  haf_points[haf_points$unique_id %notin% sampled_points$unique_id & haf_points$fate %in% observed_fates,])

# add the other had points to the desing
design_points <- rbind(design_points, haf_points)
```
...
## are sampled points in the design?
if (!all(sampled_points$unique_id %in% design_points$unique_id)) {
  sampled_points[sampled_points$unique_id %notin% design_points$unique_id,]
  warning("NOT ALL SAMPLED PLOTS ARE PRESENT IN THE DESIGN!")
}

# sampled points include 2020 plots which are not being used in this analysis
...
ratings[ratings$`Plot ID` %notin% sampled_points$unique_id,] # 4 plots from mod haf design.... need to use plotkey/PK
# this is an issue of character capitalization in PLOTID

sampled_points@data$unique_id[sampled_points@data$unique_id %notin% ratings$`Plot ID`] <- toupper(sampled_points@data$unique_id[sampled_points@data$unique_id %notin% ratings$`Plot ID`])
...
lmf_points <- subset(aim_points, grepl("^[0-9]{14}|^[0-9]{13}",aim_points@data$unique_id))

aim_points <- subset(aim_points, !grepl("^[0-9]{14}|^[0-9]{13}",aim_points@data$unique_id))
```
...
# Add the ratings to the point info and convert todata frame
