## PROPORTION2.R - DERIVES AERIAL PROPORTIONS using output from AIMWEIGHTS.
...
## set working directory
   setwd("c:/projects/fohaf/or/warner/winterAllot1")				## MUTARE
...
## Specify all PTSTALLY files - these are used to tally the total area by stratum and the total area actually sampled by stratum
   ptstally<-c("run1OR_LakeviewDO_2016_2020_sdd.gdb_ptstally_2017-12-17",
              "run2OR_LakeviewDO_2016_2020_sdd.gdb_ptstally_2017-12-17",
             "run3OR_LakeviewDO_2016_2020_sdd.gdb_ptstally_2017-12-17",
             "run4OR_LakeviewDO_2016_2020_sdd.gdb_ptstally_2017-12-17")
...
##     the sequential order in tabs.
    ...
    #abs<-c("S-4 Upland Summer Late Brood")
    tabs<-c("S-6 Winter")
    #tabs<-c("S-4 Combined Grass_Forbs")
    tabprefix<-c("WinterAllotI")
...
##  10.  Ingest, concatenate all relevant pts files output by AIMWEIGHTS.  Condition class is added to each pt and output.  A pts file is generated
    ...
    ptsfiles<-c("run1PTS_OR_LakeviewDO_2016_2020","run2PTS_OR_LakeviewDO_2016_2020")
...
##       See the decision tree below for the options here and what you should pass to GenProportions:
         ...
         if(KeyOption==2) {
         	wgts.df<-SetWgtKeys(wgts.df)		## Add fake plotkeys for pts not yet in SDD/TerrADat
                pts.spdf<-SetPtKeys(pts.spdf,wgts.df)   ## Add these fake keys to the SDD pts file. This function is inside the setwgtkeys.r file.
         }    
...
##		all plotkeys	      plotname				  1 - derive correct HAF plotname, derive & set plotkey into score$Plot.Identifier
##              only some plotkeys    plotname			          2  - same as 1, but assumes SetWgtKeys() was used to ADD fake keys where needed
##								               to both wgts.df and pts.spdf before call to GenProportions.  Pts.spdf     	
##									       will need cosmetic changes before outputing the pts shapefile at end
##									       of GenProportions.  Use this when dealing with PlotTracking input..
